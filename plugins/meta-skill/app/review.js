...
async function reviewProject(project) {
    ...
    const reviewId = await (0, project_1.nextSequencedId)(p.reviews, "quality-review");
    ...
    await (0, project_1.writeJson)(node_path_1.default.join(reviewRoot, "lint.json"), lint);
...
function implementationReview(skillText, lint) {
    const all = skillText.toLowerCase();
    return [
...
function vector(name, passed, reasoning) {
    return {
        name,
        score: passed ? 3 : 1,
        max: 3,
...
function validationChecksFromLint(lint) {
    const checks = [];
    for (const failure of lint.failures)
        checks.push(checkFromIssue(failure));
    for (const warning of lint.warnings)
        checks.push(checkFromIssue(warning));
    if (!checks.some((check) => check.name === "frontmatter_valid"))
        checks.unshift({ name: "frontmatter_valid", status: "passed", message: "SKILL.md frontmatter parsed with required fields present." });
    if (!checks.some((check) => check.name === "body_present"))
        checks.push({ name: "body_present", status: "passed", message: "SKILL.md body is present." });
    return checks;
}
function checkFromIssue(issue) {
    return {
...
function checkName(message) {
    if (/frontmatter|description|skill name|name .*folder/.test(message))
        return "frontmatter_valid";
    if (/body is missing/.test(message))
        return "body_present";
    if (/trigger context/.test(message))
        return "description_trigger_hint";
    if (/not for/.test(message))
        return "description_boundary";
    if (/long/.test(message))
        return "skill_md_line_count";
    if (/directly linked/.test(message))
...
function suggestionsFor(discovery, implementation, lint) {
    const suggestions = [];
...
function validationPercent(lint) {
