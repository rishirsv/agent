...
def handle_run(args: argparse.Namespace) -> int:
    ...
    if not session_file.exists():
        raise ValueError(f"no session found at {session_root}; run `meta-skill improve plan` first")
    ...
    result = {
        ...
        "executed_at": utc_now(),
        "status": "stub",
        "notes": "v1 skeleton; rerun `meta-skill eval run` against the working tree, then record findings manually.",
    }
    ...
    write_json(session_file, session)
        ...
        next_step=(
            "rerun `meta-skill eval run` against your candidate working tree, "
            f"then `meta-skill improve decide {args.session} --accept` or `--reject`"
        ),
    )
    return 0
