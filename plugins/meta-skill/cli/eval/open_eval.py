...
def add_open_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    parser = subparsers.add_parser("open", help="Open or list eval reports.")
...
def handle_open(args: argparse.Namespace) -> int:
    ...
    if args.list or args.history:
        if not runs:
            done("no review runs found.", path=str(runs_root), next_step="run `meta-skill eval run`")
            return 0
        for path in runs[-20:]:
            marker = "latest" if path == runs[-1] else ""
            print(f"{path.name} {marker}".rstrip())
        done(f"listed {min(len(runs), 20)} run(s).", path=str(runs_root), next_step="open a run with `meta-skill eval open --run <run-id>`")
        return 0
    ...
    if run_root is None or not run_root.exists():
        raise ValueError("no review run found; run `meta-skill eval run` first")
    report = run_root / "report.html"
    if not report.exists():
        raise ValueError(f"report does not exist: {report}")
    open_path(report)
    done("opened review report.", report=str(report), next_step=f"inspect feedback, then rerun with `meta-skill eval run --audit {relative_path(Path.cwd(), location.review_root)}` when ready")
    return 0
