...
# The standalone `meta-skill eval audit` verb has been retired. The audit
# behavior now runs from `meta-skill eval run --audit`, which calls
...
def handle_audit(args: argparse.Namespace) -> int:
    ...
    if not prompts:
        ...
        suggestions.append("Add 3-5 realistic prompts or rerun `meta-skill eval init --from-skill-spec docs/spec.md`.")
    if not runs:
        issues.append("No run evidence yet.")
        suggestions.append("Run `meta-skill eval run --compare none`.")
