---
name: improve-skill
description: "Use when a user asks to improve a skill, prompt doctor a skill, review a skill, fix my skill, update a skill, or make evidence-backed changes to an existing reusable skill: run/read review.md, fix validation issues, patch SKILL.md, references, scripts, or assets, and incorporate eval, trace, or human-feedback findings; not for creating new skills, running evals, broad rewrites without evidence, packaging, installing, or publishing."
---
...
| Codex child-thread/worktree evidence loops for candidate improvements | [codex-threads-runner.md](../../references/codex-threads-runner.md) |
...
- `.meta-skill/review.md` is the single review artifact for deterministic validation evidence and a Quality-page worksheet.
...
- If the user requires zero writes, say that the full review artifact cannot be produced under that constraint, then do a manual read-only review or ask permission to write `.meta-skill/review.md`.
- Pick one route:
...
- Require evidence before patching: validation output, eval run ID, eval ID, test failure, trace, saved evidence file, or human feedback. Each edit should cite at least one evidence reference.
- Do not invent proof, auto-apply edits, package, install, or publish. Never fabricate validation rows, deterministic test status, scores, run IDs, or evidence files; if evidence is missing, say what is missing and keep the finding evidence-scoped.
- Treat completed eval execution as evidence only, not pass proof. Before editing from it, name what ran, what compact rows or child-thread/worktree evidence exists, and what the evidence shows.
...
- Review the changed files and rerun any relevant deterministic tests or evidence collection after edits.
- For iterative improvement, prefer candidate worktrees and child threads. The parent recommends accept, revise, rerun, or discard from compact evidence, and asks for approval before promoting candidate changes into source.
