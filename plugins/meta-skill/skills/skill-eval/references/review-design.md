...
`meta-skill eval run --audit` runs an audit pass before the next run. The audit itself is read-only — it inspects the workspace and reports the next useful step, such as:
