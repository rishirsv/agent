...
Measure a reusable skill with `.meta-skill/cases/` and `.meta-skill/runs/`. This lane sets up manually authored cases, runs Codex App Server-backed evidence collection, inspects fact-backed reports, imports feedback, and runs optional App Server-backed judges. It does not rewrite, package, or edit the skill.
...
- Judges read frozen run cases, final output, and the compact trajectory summary when present. Threshold failures are recorded as check observations; the report does not compute pass/fail.
- `run`, `judge`, `feedback`, `lint --run`, and `decide` append facts to `facts.jsonl`; reports compute from facts and print Markdown or JSON without persisting report files.
...
- `rpc.jsonl` is the durable raw App Server trace for a case. `trajectory.json` is the normalized turn evidence used by reports and judge context.
- `meta-skill run --type` accepts only `R`, `F`, and `G`. Current eval guidance uses manually authored cases, one execution source per run, read-only App Server evidence, and printed reports.
...
Inspect `meta-skill report <run-id> --json` for machine-readable evidence, then drill into `facts.jsonl`, `cases/<case>/case.md`, `cases/<case>/rpc.jsonl`, `cases/<case>/trajectory.json`, and `cases/<case>/final.md`.
