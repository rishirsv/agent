...
class CodexRunner:
    ...
    def run_role(
        ...
        *,
        ...
        if self.keep_workspace:
            stage = Path(tempfile.mkdtemp(prefix="skillfactory-review-"))
            return self._run_in_stage(stage=stage, skill_root=skill_root, prompt=prompt, evidence_dir=evidence_dir)
        with tempfile.TemporaryDirectory(prefix="skillfactory-review-") as tmp:
    ...
    def _build_prompt(self, prompt: dict[str, Any]) -> str:
        ...
        return "\n".join(
            [
                "You are running a non-interactive skill review role for the skillfactory CLI.",
