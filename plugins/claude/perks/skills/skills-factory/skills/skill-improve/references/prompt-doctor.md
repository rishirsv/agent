...
- Add positive-null behavior to review skills.
...
The `skillfactory improve plan` → `run` → `decide` skeleton records the bounded-edit *intent and decision*. This reference is the substantive doctrine the agent applies inside that loop:
...
- Between `run` and `decide`, apply the **Surgical Update Rules** when carrying out the actual edit by hand, then rerun `skillfactory review run` to test it.
