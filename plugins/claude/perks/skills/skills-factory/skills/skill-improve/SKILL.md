---
name: skill-improve
description: Use when reviewing, auditing, patching, or surgically editing an existing skill from review evidence with the prompt-doctor loop and bounded-edit discipline; not for creating new skills, autonomous optimization, or silent source promotion.
---

# Skill Improve

Edit an existing skill with precision. Consume `skill-review` evidence (or a direct user complaint), translate it into the smallest useful change, test the candidate, record the human decision. This lane owns the prompt-doctor loop, surgical update rules, and the review-vs-edit mode split.
...
| Apply the shared CLI conventions across `skill-create`, `skill-review`, and `skill-improve` | sibling `skill-create` skill's `references/cli-conventions.md` |
...
- Use the project spec at the skill's project root (`skills/skills-factory/docs/skill-improve-spec.md`) as the source of truth.
- Use the shared `skillfactory` CLI for commands, with improvement commands under `skillfactory improve ...`.
- Require review evidence — a `skill-review` run, traces, or a concrete observed failure — before proposing a patch.
...
```bash
skillfactory improve plan <project> [--from-run <review-run-id>]
skillfactory improve run <project> --session <session-id>
skillfactory improve decide <session-id> --accept   # or --reject
```
...
- `run` records the candidate intent. In v1, real candidate orchestration is not built; rerun `skillfactory review run` by hand against the candidate working tree.
...
- Do not replace `skill-review`; rerun review gates to test candidate changes.
