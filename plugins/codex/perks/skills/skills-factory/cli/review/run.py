...
def add_run_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    parser = subparsers.add_parser("run", help="Run review prompts.")
...
def handle_run(args: argparse.Namespace) -> int:
    ...
    if should_open and stdout_is_tty():
        open_path(report)
    next_step = "open the report with `skillfactory review open`, then rerun with `skillfactory review run --audit` when ready"
    if kept_workspaces:
