...
def handle_decide(args: argparse.Namespace) -> int:
    ...
    if decision == "accept":
        next_step = "apply the candidate edit by hand, rerun `skillfactory review run`, then commit"
    else:
        next_step = f"plan a different angle with `skillfactory improve plan {args.project}`"
