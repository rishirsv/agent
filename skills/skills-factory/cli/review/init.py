...
def add_init_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    parser = subparsers.add_parser("init", help="Create a review workspace.")
...
def handle_init(args: argparse.Namespace) -> int:
    ...
    prompt_note = f" seeded {len(prompts)} prompts" if prompts else " created empty prompt set"
        ...
        next_step=f"review {relative_path(Path.cwd(), review_root / 'prompts.yaml')}, then run `skillfactory review run`",
    )
    return 0
