...
| Apply the shared CLI conventions across `skill-create`, `skill-review`, and `skill-improve` | [cli-conventions.md](references/cli-conventions.md) |

## CLI

Scaffolding and validation are CLI verbs, not scripts in this payload:

- `skillfactory create ...` — scaffold a clean project wrapper, spec, portable `SKILL.md`, and `agents/openai.yaml` (see step 3).
- `skillfactory validate <path>` — run structural validation (see step 5).

The shared `skillfactory` binary is repo infrastructure, not part of the portable skill payload. Both verbs are implemented in the engine at `skills/skills-factory/cli/` and run through the `bin/skillfactory` shim. The CLI expects Python 3 with PyYAML available; if PyYAML is missing, install it in the local development environment before running `bin/skillfactory`.
...
`skillfactory create` creates a clean wrapper, a project spec, a portable `SKILL.md`, and `agents/openai.yaml`. It is not a skill author. It should not infer review mode, source controls, artifact handling, tone, output contract, or failure handling from flags. Do that design work in the agent reasoning loop using the references in this skill.
...
Run structural validation from the repo root with `skillfactory validate`, pointing it at the project wrapper and portable skill folder.

```bash
skillfactory validate <project-dir> --portable-dir <skill-dir>
```
...
For existing-skill edits driven by review evidence, hand off to the sibling `skill-improve` skill. It owns the prompt-doctor loop, surgical update rules, finding shape, and the bounded-edit cycle that wraps around `skillfactory review` evidence.
