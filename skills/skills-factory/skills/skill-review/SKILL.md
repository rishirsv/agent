---
name: skill-review
description: Use when setting up, running, auditing, or interpreting local Skills Factory reviews for reusable skills with `skillfactory review`; not for rewriting skills, optimizing candidates, packaging, installing, or promoting releases.
---

# Skill Review

Help a user measure a reusable skill with the `skillfactory review` CLI. This skill guides review setup, prompt design, run interpretation, evidence inspection, and reviewer feedback. It does not edit, optimize, package, install, or promote the skill under review.

## Route First

- Use this skill for review scaffolds, review prompt design, running `skillfactory review ...`, interpreting reports, baseline handling, feedback capture, and manual review workflow.
...
Use `skillfactory review init . --from-skill-spec docs/my-skill-spec.md` when a skill-create project spec contains a structured review seed. If the skill clearly needs multiple review tracks, initialize with `--name <review-name>`; otherwise keep the default flat `reviews/` layout.

`skillfactory review run` on an uninitialized project auto-creates `review.yaml` and `prompts.yaml` and then proceeds, printing one stderr line so the implicit step is visible. Run `init` first when you want to pre-seed prompts from a spec.

Use `skillfactory review run --audit` to audit the workspace and then run in the same command. The standalone `review audit` verb is gone.
...
Shared CLI conventions (exit codes, output channels, common flags, human gates) live in the sibling `skill-create` skill's `references/cli-conventions.md` and apply to every `skillfactory ...` command.
...
Use `skillfactory review run` with `default_compare: auto` unless the user has a reason to override it:
...
- next useful step, such as add prompts, inspect traces, import feedback, accept a baseline, add a check, or rerun with `skillfactory review run --audit`
